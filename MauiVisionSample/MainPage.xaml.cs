...
public partial class MainPage : ContentPage
{
    ...
    async Task<byte[]> PickPhotoAsync()
    {
        ...
        // Take a screenshot of the app to use as a frame for object detection
        IScreenshotResult screen = await Screenshot.Default.CaptureAsync();

        // get a graphics image from the screenshot
        IImage image;
        Assembly assembly = GetType().GetTypeInfo().Assembly;
        using (Stream stream = await screen.OpenReadAsync())
        {
            image = PlatformImage.FromStream(stream);
        }

        // save the iimage to memory stream as a png so that Yolo can read it
        ...
        return Utils.HandleOrientation(memStream.ToArray());
    }
